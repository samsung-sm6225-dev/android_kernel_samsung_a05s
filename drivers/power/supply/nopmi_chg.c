				...
				const int value)
{
	...
	if (value) {
		...
		if (!(nopmi_chg->batt_full_capacity_soc == nopmi_chg->Current_SOC  && soc_full  == 1)){
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		rc = power_supply_get_property(nopmi_chg->main_psy, psp, pval);
		pr_err("main_chgdet = %d \n", nopmi_chg->main_chgdet);
		if(!nopmi_chg->main_chgdet){
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		nopmi_chg->Current_SOC=  pval->intval; 
#ifdef NOPMI_FULL_IN_ADVANCE
          ...
          char *buf)
{
	...
	pr_err("main_chgdet = %d,batt_soc_rechg = %d \n", chg->main_chgdet,chg->batt_soc_rechg);
	if ((soc_full == 1) && (chg->input_suspend == 1) && chg->main_chgdet) {
		...
		union power_supply_propval *pval)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_ONLINE:
			if (strcmp(nopmi_chg->batt_full_capacity,"SLEEP") && nopmi_chg->batt_full_capacity_soc == STOP_CHARGE_SOC && soc_full == 1) {
				pval->intval = 0;
				break;
			}
			if(!nopmi_chg->main_psy)
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		if (strcmp(nopmi_chg->batt_full_capacity,"SLEEP") && nopmi_chg->batt_full_capacity_soc == STOP_CHARGE_SOC && soc_full == 1) {
			pr_info("battery protect set usb online  0\n");
				val->intval = 0;
				break;
		}
...
static void nopmi_batt_capacity_check_work(struct work_struct *work)
{
	...
	int rc = 0;
	int soc = 0;
	int charge_protect_stop_charge;
	...
	charge_protect_stop_charge = nopmi_chg->batt_full_capacity_soc;

	if(charge_protect_stop_charge != 0 && charge_protect_stop_charge != 100){
		if ((soc < charge_protect_stop_charge && soc_full == 0) || (soc >= charge_protect_stop_charge && soc_full == 1))
		{
		...
		if(soc >= charge_protect_stop_charge) {
			soc_full = 1;
			pval.intval = 0;
			rc = power_supply_set_property(nopmi_chg->usb_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
			rc = nopmi_set_prop_input_suspend(nopmi_chg, 1);
  	...
  	if (event == TCP_NOTIFY_TYPEC_STATE)
  	{
  		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
  			...
  			&& noti->typec_state.new_state == TYPEC_UNATTACHED) {
				soc_full = 0;
				g_nopmi_chg->input_suspend  = 0;
				rc = vote(g_nopmi_chg->fcc_votable, CHG_INPUT_SUSPEND_VOTER, false, 0);
				rc |= vote(g_nopmi_chg->usb_icl_votable, CHG_INPUT_SUSPEND_VOTER, false, 0);
				rc |= vote(g_nopmi_chg->chg_dis_votable, CHG_INPUT_SUSPEND_VOTER, false, 0);
  				pr_err("USB Plug out soc_full:%d input_suspend %d\n", soc_full, g_nopmi_chg->input_suspend);
  		}
  	}
